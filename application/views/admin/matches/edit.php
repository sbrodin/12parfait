...
<a href="<?= site_url('onarie/matches/fixture') ?>" class="btn btn-sm btn-outline-primary m-b-1"><?= $this->lang->line('add_match');?></a><br/>
<?= validation_errors() ?>
...
<?php else : ?>
    ...
    foreach ($matches_fixtures as $key => $matches_fixture) {
        if($matches_fixture->fixture_name!==$fixture_name) {
            echo '</div><br/><div>';
            echo '<a href="'.site_url('onarie/fixtures/edit/'.$matches_fixture->fixture_id).'" class="btn btn-sm btn-outline-primary m-b-1">'.$this->lang->line('edit_fixture').' "'.$matches_fixture->fixture_name.'"</a><br/>';
